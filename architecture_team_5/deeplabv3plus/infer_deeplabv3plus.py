...
def preprocess_image_for_deeplabv3plus(img_raw: np.ndarray, preprocess_mode="basic") -> np.ndarray:
...
def resize_for_deeplabv3plus(img01: np.ndarray, target_size) -> np.ndarray:
    resized = cv2.resize(
        img01,
...
def run_deeplabv3plus_inference(image: np.ndarray, model, device, cfg):
    ...
    img_pp = preprocess_image_for_deeplabv3plus(img_uint8, preprocess_mode=preprocess_mode)
    img_rs = resize_for_deeplabv3plus(img_pp, target_size=target_size)
